...
using SMEAppHouse.Core.ScraperBox.FreeIPProxies.Providers.Base;

#pragma warning disable 168

namespace SMEAppHouse.Core.ScraperBox.FreeIPProxies.Providers
{
    public class PremProxyComCartridge<TContentCrawler> : IPProxyCartridgeBase<TContentCrawler>
        where TContentCrawler : IContentCrawler<ICrawlerOptions, ICrawlerResult>
    {
        public PremProxyComCartridge(TContentCrawler contentCrawler, int startPageNo = 0)
                    : base(contentCrawler, "https://premproxy.com/list/{PAGENO}.htm",
                          new PageInstruction()
                          {
                              PadCharacter = '0',
                              PadLength = 2,
        ...
        protected override void ValidatePage(string content)
        {
            try
            {
                ...
                HtmlNode target = SMEAppHouse.Core.ScraperBox.Helper.GetNodeByAttribute(document, "table", "id", "proxylistt");
        ...
        protected override void ParseProxyPage(string content)
        {
            ...
            doc.LoadHtml(content);
            var document = doc.DocumentNode;
            var target = SMEAppHouse.Core.ScraperBox.Helper.GetNodeByAttribute(document, "table", "id", "proxylist");

            if (target == null) return;

            var lines = SMEAppHouse.Core.ScraperBox.Helper.GetNodeCollection(target, "tr");
            ...
            lines.ToList().ForEach(e =>
            {
                ...
                if (!string.IsNullOrEmpty(countryPartial))
                {
                    ...
                    var countriesEnum = SMEAppHouse.Core.ScraperBox.FreeIPProxies.Rules.FindCountry(countryPartial)?.Item1;
                    if (countriesEnum != null)
                        proxy.Country = (SMEAppHouse.Core.CodeKits.Rules.WorldCountriesEnum)countriesEnum;
                }
