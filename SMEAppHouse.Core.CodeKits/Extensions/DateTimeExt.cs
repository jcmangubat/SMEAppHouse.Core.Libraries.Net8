...
$"{timeSpan.Hours} hours {timeSpan.Minutes} minutes and {timeSpan.Seconds} seconds";
    ...
    public static string ConvertFromEuToUsDateFormat(this string euDate)
    {
        var gbCulture = "en-GB";
        var usCulture = "en-US";
        var date = DateTime.Parse(euDate, new CultureInfo(gbCulture, false));

        var usDate = date.ToString(new CultureInfo(usCulture, false));
