...
namespace SMEAppHouse.ScraperBox.FreeIPProxies.Providers
{
        ...
        where TContentCrawler : IContentCrawler<ICrawlerOptions, ICrawlerResult>
    {
        ...
        protected override void ValidatePage(string content)
        {
            try
            {
                ...
                HtmlNode target = Common.Helper.GetNodeByAttribute(document, "table", "id", "proxylistt");
        ...
        protected override void ParseProxyPage(string content)
        {
            ...
            doc.LoadHtml(content);
            var document = doc.DocumentNode;
            var target = Common.Helper.GetNodeByAttribute(document, "table", "id", "proxylist");

            if (target == null) return;

            var lines = Common.Helper.GetNodeCollection(target, "tr");
            ...
            lines.ToList().ForEach(e =>
            {
                ...
                if (!string.IsNullOrEmpty(countryPartial))
                {
                    ...
                    var countriesEnum = Rules.FindCountry(countryPartial)?.Item1;
                    if (countriesEnum != null)
                        proxy.Country = (Rules.CountriesEnum)countriesEnum;
                }
