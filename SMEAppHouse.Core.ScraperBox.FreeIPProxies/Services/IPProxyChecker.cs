...
namespace SMEAppHouse.Core.ScraperBox.FreeIPProxies.Services
{
        ...
        protected override void ServiceActionCallback()
        {
            ...
            foreach (var prx in opnPrxs)
            {
                var tsk = new Task(() =>
                {
                    ...
                    OnIPProxyChecked?.Invoke(this, new EventHandlers.IPProxyCheckedEventArgs(isValid, prx));
                });
                tsk.Start();
                prxTaskList.Add(tsk);
            }
            ...
            lock (ProxyBucket)
            {
                var removedProxies = ProxyBucket.Where(proxy => proxy.CheckStatus == Models.IPProxy.CheckStatusEnum.CheckedInvalid).ToList();
                var removedCnt = removedProxies.Count;
                foreach (var proxy in removedProxies)
                {
                    ProxyBucket.Remove(proxy);
                }
                OnIPProxiesChecked?.Invoke(this, new EventHandlers.IPProxiesCheckedEventArgs(removedCnt));
            }
